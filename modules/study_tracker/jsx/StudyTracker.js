...
class SiteFilter extends React.Component {
    ...
    render() {
        ...
        for (centerID in sites) {
          options.push(<option key={centerID} value={centerID}>{sites[centerID]}</option>);
        }
      return (
            <select className="form-control input-sm" onChange={this.props.filterSites}>
                {options}
            </select>
        );
    }
...
class CohortFilter extends React.Component {
    ...
    render() {
        ...
        for (cohortID in cohorts) {
            options.push(<option key={cohortID} value={cohortID}>{cohorts[cohortID]}</option>);
        }
        return (
            <select className="form-control input-sm" onChange={this.props.filterCohorts}>
...
class SideBarCandContent extends React.Component {
    render () {
        ...
        if (this.props.row.dxReappraisal) {
          let url = loris.BaseURL + `/candidate_parameters/?candID=${candid}&identifier=${candid}#diagnosisReappraisal`;
          content.push(
            <a
              href={url}
              className="small"
...
class PSCIDCell extends React.Component {
    render() {
        ...
        if (this.props.dxReappraisal) {
            feedBackIcons.push(<span className="glyphicon glyphicon-random" style={style}/>)
        }
        return (
            <td
                className='PSCIDCell'
...
class StudyTracker extends React.Component {
    ...
    loadDataFromServer() {
        $.get(GET_DATA_URL, {data: "all"}, function(data, status) {
            console.log(data);
           if (status === "success") {
               let cohorts = Object.create(null),
                 visitLabels = [],
                 feVisitLabels = [],
                 rows = [],
                 sites = Object.create(null);
