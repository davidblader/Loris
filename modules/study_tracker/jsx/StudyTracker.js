...
class SiteFilter extends React.Component {
  render() {
    ...
    let centerID;
    for (centerID in sites) {
      if (sites.hasOwnProperty(centerID)) {
        options.push(
          <option key={centerID} value={centerID}>{sites[centerID]}</option>
        );
      }
    }
    return (
      <select className="form-control input-sm"
              onChange={this.props.filterSites}>
        {options}
      </select>
    );
  }
  ...
  render() {
    return (
      <input className="form-control input-sm"
             ...
             onChange={this.props.filterCand}
...
class CohortFilter extends React.Component {
  render() {
    ...
    let cohortID;
    for (cohortID in cohorts) {
      if (cohorts.hasOwnProperty(cohortID)) {
        options.push(
          <option key={cohortID} value={cohortID}>{cohorts[cohortID]}</option>
        );
      }
    }
    return (
      <select className="form-control input-sm"
              onChange={this.props.filterCohorts}>
...
class FilterDisplayToggle extends React.Component {
  render() {
    let glyphClass = 'down';
    ...
    return (
      ...
      >
        Filters &nbsp;
        <span className={"glyphicon glyphicon-chevron-" + glyphClass}/>
      </h5>
    );
  }
}
    ...
    return (
      <div>
        ...
        <div className="legend-entry">
          ...
          <span>
            No deadline for visit. This is the initial
...
class SideBarCandInstContent extends React.Component {
  render() {
    ...
    for (let sg in data) {
      if (data.hasOwnProperty(sg)) {
        content.push(<h6>{sg}</h6>);
        for (let t in data[sg]) {
          if (data[sg].hasOwnProperty(t)) {
            let inst = data[sg][t];
            ...
            if (inst.ddeCompletion === "Complete") {
              flagCompletion = (
                <span>
                  <span className="glyphicon glyphicon-ok complete"></span>
                  <span className="glyphicon glyphicon-ok complete"></span>
                </span>
              );
            } else if (inst.completion === "Complete") {
              flagCompletion = <span className="glyphicon glyphicon-ok complete"/>;
            } else {
              flagCompletion = <span className="glyphicon glyphicon-alert deadline-past"/>;
            }
            let conflicts = [];
            if (inst.conflicts) {
              conflicts.push(
...
class SideBarCandContent extends React.Component {
  render() {
    ...
    if (this.props.row.dxReappraisal) {
      let url = loris.BaseURL + `/candidate_parameters/?candID=${candid}` +
        `&identifier=${candid}#diagnosisReappraisal`;
      content.push(
        <a
          href={url}
          className="small"
        >
          <span className="glyphicon glyphicon-random" style={iconColor}/>
          <em>&nbsp;Participant has had a diagnosis reappraisal</em>
        </a>
      );
    }
...
class SideBarVisitContent extends React.Component {
  render() {
    ...
    let visitsSortable = [];
    let dataSortable = [];
    // Loop through rows and put visit data into sortable arrays
    ...
    if (visit.cohort === this.props.currentCohort || this.props.currentCohort === "all") {
      ...
      return (
        <td className={visit.visitLabel} style={bgColor}>
          <div onClick={e => this.props.showCandInstFocus(sidebarArgs, e)}
...
class PSCIDCell extends React.Component {
  render() {
    ...
    if (this.props.dxReappraisal) {
      feedBackIcons.push(
        <span className="glyphicon glyphicon-random" style={style}/>
      );
    }
    return (
      <td
...
class StudyTracker extends React.Component {
    ...
    $.get(GET_DATA_URL, {data: "all"}, function(data, status) {
      if (status === "success") {
        ...
        for (let r in data.tableData) {
          if (data.tableData.hasOwnProperty(r)) {
        ...
        for (let c in data.cohorts) {
          if (data.cohorts.hasOwnProperty(c)) {
            cohorts[c] = data.cohorts[c];
          }
        }
        this.setState({cohorts: cohorts});

        for (let s in data.sites) {
          if (data.sites.hasOwnProperty(s)) {
            sites[s] = data.sites[s];
          }
        }
        this.setState({sites: sites});

        for (let i = 0; i < data.visitLabels.length; i++) {
  ...
  showCandInstFocus(sidebarArgs, event) {
    ...
    if (sidebarArgs.sessionID > 0) {
      $.get(GET_DATA_URL, {data: "instruments", sessionID: sidebarArgs.sessionID},
  ...
  showSideBar() {
    ...
    let selectors = ".study-tracker-table, .study-tracker-header, .filter-container";
  ...
  filterCand(event) {
    this.setState({
      filterCandText: event.target.value
    });
  }
  ...
  renderRow(row) {
    if (this.rowHasCurrentCohortVisit(row) &&
      ...
      row.pscid.startsWith(this.state.filterCandText.toUpperCase())
    ) {
      return true;
    }
    return false;
  ...
  render() {
    ...
    if (dataRows.length === 0) {
      noMatch = <p>No participants match the current search parameters.</p>;
    }
    ...
    return (
      <div className="StudyTracker">
        ...
        <div className="row study-tracker-header">
          <div className="col-md-6">
            <h5>Total Rows: {dataRows.length}</h5>
          </div>
          <div className="col-md-6">
            <FilterDisplayToggle
